...
import static com.example.ONDC.sellerApp.AIProductCataloging.enums.ChatCompletionPrompts.*;
  ...
  private List<AIShortLearningDTO> getMessageList(String description, Integer category, String title) {
    return switch (category) {
      ...
      case 4 -> Arrays.asList(
        AIShortLearningDTO.builder().role(SYSTEM_ROLE).content(String.format(ENHANCE_DESCRIPTION_TONALITY_SETUP_FOOD.getValue(),description,title)).build(),
